...
状态：Phase 9 实施中；首个可发布双向切片正在完成发布门禁，尚未发版
最后更新：2026-09-22
...
- Step 3/4 首发子集已形成：Apple 纯 Swift `organic-v3` 双向规则当前覆盖苯，以及无支链 C1–C10 烷烃、单烯烃、单炔烃、一元醇、醛、酮和一元羧酸；其余结构明确返回 `unsupported`；
...
1. 在 `NativeCore` 建立图分析基础：连通分量、环系、主链/母体候选、官能团优先级、编号和立体描述符；
2. 按 NISPO 的规则阶段移植无环、单环、杂环、稠环和螺环命名；
3. 用 `ChemDocument` 的稳定原子 ID 保留名称片段与来源原子的映射，便于错误定位；
4. 每个生成名称都送入开发期参考 OPSIN 做差分回译；产品运行时由 Swift 原生反向解析器执行同一检查；
5. 不支持结构返回具体失败阶段，不退化为分子式、SMILES 或数据库标题。

退出条件：Apple 冻结集中“返回成功”的结果 100% 回译为等价分子图；另行报告覆盖率，禁止通过大量拒绝伪造准确率。
