...
- 2026-09-23：在已有无环羧酸母体和取代基流水线中加入末端氨基前缀，不增加整图查表；C3–C8 直链所有 27 个氨基位次均与固定 NISPO 0.1.7 名称逐字一致且 OPSIN 回译等价，另有支链、双氨基和氨基/羟基组合回归。原有 73 个羧酸系统样本仍 73/73 OPSIN 等价，但其既存名称逐字一致数仅 7/73，不能据此宣称 `acyclic-parent` 完成。官方 1,296 例可命名数仍为 134，逐字一致从 116 增至 118，134/134 OPSIN 等价；下一步继续补齐主官能团优先级、取代基递归与现存名称文字差异；
