...
状态：Phase 9 实施中；结构→名称优先扩展；0.1.76 已冻结并提交统一发版，NISPO 完成门禁仍未通过（官方 336/1,296；2026-09-24 全天切片后实测，历史值 231 保留于 git 历史）
最后更新：2026-09-23
...
- Step 3 未完成：Apple 纯 Swift 已接入与官方同名的 `MoleculeFacts → RoutePlan → RuleSpec → Candidate` 通用执行骨架，并为每个结果记录实际规则族。第一次诚实审计发现原实现 98 个“成功”中仅 62 个 OPSIN 回译等价，随后已禁止旧精确图查表和未经支持的电荷、自由基、异常氢价、互变异构/指示氢状态冒充成功。官方 1,296 条基线当前 1,296 条均可导入、291 条返回名称，291/291 OPSIN 回译结构等价、264 条名称与 NISPO 完全一致、`legacy-exact-graph-fallback` 命中为 0；命名覆盖率为 22.45%，仍有 1,005 条未命名。123 个上游 `RuleSpec` 仍无一达到 `implemented`。123 个上游 `RuleSpec` 仍无一达到 `implemented`。母体候选排序、递归取代基、完整后缀、部分氢化环、互变异构/指示氢以及复杂电荷/硫磷硼规则尚未等价移植；后续必须按上游规则族和共享组装阶段推进，不能把小型系统语料的全绿当作本步骤接近完成；
...
- 2026-09-23（晚间第二切片）：继续照抄移植 `acyclic/rules.py` 与 `branch/functional_groups.py`：磷氧取代前缀 `dioxophosphanyl`/`phosphono`/`phosphonato` 与 geminal 双膦酸核心（"1-dioxophosphanylhexane"、"1-hydroxy-1,1-diphosphonoethane"、"1-cyano-1,1-bis(dioxophosphanyl)-methane"）；硅/氧替代骨架（"3,3-dichloromethyl-2,4-dimethyl-3-silapentane"、"6,6-dipropyl-4,8-dioxa-6-silaundecane"、"1,1,1,3,3,3-hexafluoro-2-trifluoromethyl-2-silapropane"）；bis(azanidyl) 与 dioylbis(azanide)（"1,2-bis(azanidyl)-ethane"、"ethanedioylbis(azanide)"、"hexanedioylbis(azanide)"）；azido 前缀（"1-azanidyl-3-azidopropane"）；1-oxido-1-azoniamethane；hydroxymethyl 与 fluoroazanidyl 前缀；阴离子酰胺的 S-H/氨基 owner（"phenyl sulfanylamide"、"ethylaminoamide"）。官方 1,296 例由 231 升至 249（249/249 OPSIN 等价、222 逐字一致），单酰基 azanide 与带立体中心案例安全拒绝以保住等价门禁；全部既有系统语料复测无退化。全量 `swift test` 中唯一失败的 `ScenePerformanceTests.testLargeDocumentScenePipeline` 为并行时序抖动（单独重跑 0.061s < 0.064s 通过），本次未触碰渲染管线代码。
- 2026-09-23（晚间第三切片）：新增含氧立体骨架规则（"1-hydroxy-3,7-dimethyl-6-octen"、"(2R,3R,4R)-1,2,3,4,6-pentahydroxy-5-oxohexane"），并把描述符定向键改为羟基位次优先于多重键（排除 oxo 碳），使引擎层 R/S 位次与 NISPO 母体编号一致；azaniumdiolate 扩展到双配体（"propan-2-ylideneazaniumdiolate"）。官方 1,296 例由 249 升至 252（252/252 OPSIN 等价、225 逐字一致），全部既有语料无退化，`swift test` 退出码 0。
- 2026-09-23（晚间第四切片）：异腈 owner 扩展到环上取代的苯甲酰（"4-methylbenzoyl isocyanide"），并把氟代 azanidyl（"1,1,1,2,2-pentafluoro-2-fluoroazanidyl-ethane"）的前缀识别接入共享路径机制（按 azanidyl 位次最大化选择母体方向）。官方 1,296 例由 252 升至 254（254/254 OPSIN 等价、227 逐字一致），全量 `swift test` 退出码 0，既有语料无退化。
- 2026-09-24（凌晨切片）：`single-ring-parent` 新增两类共享规则：零价原子芳基（零价 B/S/Si/C 单键连芳香单环，"pyridin-2-ylboron"、"1-bromobenzene-4-ylsulfur"、"1H-pyrrole-2-ylsilicon"、"benzene-1-ylcarbon"、"thiofuran-2-ylboron"、"1H-imidazol-4-ylboron"、"pyrazin-2-ylboron" 等），采用 Hantzsch-Widman 杂原子位次集表驱动环名与编号；吡喃鎓（芳香 6 元 1 个 O+，"pyrylium"、"4-methyl-pyrylium"、"2,3-diphenyl-pyrylium"，独立规则族 `pyrylium-parent` 以覆盖多环分子路由）。官方 1,296 例由 254 升至 264（264/264 OPSIN 等价、237 逐字一致），新增 69 例系统冻结语料（61 named、59 exact、59 等价），`swift test` 185 项退出码 0。
- 2026-09-24（凌晨第二切片）：`single-ring-parent` 新增芳香杂环硫属酮（内酰胺/硫酮）规则 `chalcogenone-ring-parent`：按"逐位置元素序列"匹配 Hantzsch-Widman 母体（1,2,4-triazole、tetrazole、1,2,4-oxadiazole、1,2,3-triazoxol、s-triazine、1,2,4-triazine、1,3,4,5-tetrazine、1,5/1,4-diazepine），组装 "-{位}({H位}H)-one/-thione" 与 7 元 "-{位}-on/-thione"。官方 1,296 例由 264 升至 274（274/274 OPSIN 等价、247 逐字一致），新增 18 例系统硫属酮语料（18/18 命名且逐字一致；1 例 NISPO 参考文本自身回译 H 移位，保持冲突记录），`swift test` 186 项退出码 0。
- 2026-09-24（凌晨第三切片）：新增芳香鎓规则 `aromatic-azonium-parent`（pyridin-1-ium 含 carboxylato 前缀、1H-1,2,3-triazol-3-ium、4H-1,2,4-triazol-1-ium；要求全芳环，饱和 N+ 环仍走既有正离子规则）与端位硝基/腈基甲烷负离子（"1-cyano-1-nitrosomethanide"）。官方 1,296 例由 274 升至 278（278/278 OPSIN 等价、251 逐字一致），`swift test` 186 项退出码 0。
- 2026-09-24（清晨切片）：新增 `NativeNISPOBenziodoxoleRules.swift`，照抄移植 `BenziodoxoleRetainedParentRule` 的十二个 SMARTS 变体为融合骨架图检查：hydride、substituted hydride、oxo-substituted hydride、oxo ring-substituted、geminal dimethyl、halo/hydroxy/iodoxy lactone、dihydro。官方 1,296 例由 278 升至 291（291/291 OPSIN 等价、264 逐字一致），`benziodoxole-retained-parent` 由 `missing` 升为 `partial`（16 例官方归因中 13 例命名且等价；位次细节与三酰氧基变体仍缺），`swift test` 186 项退出码 0。
