...
- Step 3 未完成：Apple 纯 Swift 已接入与官方同名的 `MoleculeFacts → RoutePlan → RuleSpec → Candidate` 通用执行骨架，并为每个结果记录实际规则族。第一次诚实审计发现原实现 98 个“成功”中仅 62 个 OPSIN 回译等价，随后已禁止旧精确图查表和未经支持的电荷、自由基、异常氢价、互变异构/指示氢状态冒充成功。官方 1,296 条基线当前 1,296 条均可导入、62 条返回名称，62/62 OPSIN 回译结构等价、54 条名称与 NISPO 完全一致、`legacy-exact-graph-fallback` 命中为 0；命名覆盖率仍仅 4.78%。按上游实际命中规则归因，最大缺口依次包括 `single-ring-parent`（208 例，仅 12 例原生命名）、`kekulized-generic-polycyclic-parent`（137 例，0 例）和 `generated-fused-parent`（94 例，0 例）。母体候选排序、递归取代基、完整后缀、部分氢化环、互变异构/指示氢以及复杂电荷/硫磷硼规则尚未等价移植，因此本步骤距离退出条件仍很远；
...
- 2026-09-23：沿 `single-ring-parent` 的保留杂环母体与环编号管线，新增饱和单 N/O/S 杂环的端点取代命名：以杂原子为 1 位、双向比较最低取代位次、组合 λ 位次及 NISPO 的 homopiperidin 保留母体。图枚举冻结的 258 个结构全部名称文字与 NISPO 一致且 OPSIN 回译等价；不按单个结构查表。该规则族仍为 `partial`，多杂原子、多取代、复杂支链与立体取向仍缺；
- `scripts/verify_nispo_native_completion.py` 是统一硬门禁：123 个规则族必须全部为 `implemented` 后才会执行官方及各系统语料的全量命名、精确名称与 OPSIN 等价差分。当前 120 个 `missing`、3 个 `partial`，门禁预期失败并阻止发版；
