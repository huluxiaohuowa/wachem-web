...
- `scripts/generate_nispo_aromatic_bicyclic_corpus.py <NISPO-0.1.7-checkout>` 系统生成 445 个芳香双环及 N/O/S 骨架置换结构，并冻结上游实际 producer。Apple 共享芳香键 Kekulé 视图与双环图编号管线当前 435/445 原生命名，435/435 OPSIN 图等价，420 个与 NISPO 原文一致；上游 `kekulized-generic-polycyclic-parent` 的 443 例中有 433 例命名、420 例逐字一致。另 10 例包含芳香原子间显式桥双键，导入器现保留该键级，但命名阶段安全拒绝，不能把其改写为普通芳香键。此子域仍缺稠合保留母体取向、显式桥键处理和多于两环的通用规则；
...
- 2026-09-23：新增共享芳香键 Kekulé 视图，复用双环桥编号与多 N/O/S 骨架置换前缀；445 个系统芳香双环样本有 435 个原生命名、420 个逐字一致，成功结果 435/435 OPSIN 等价，10 个显式芳香桥双键暂安全拒绝。共享 SMILES 导入器保留显式双/三键键级，已有稠环模板回归已修复；原有 464 例双环及 1,264 例双环多烯基准数字不变。官方全量仅从 125 提升到 134/1,296（116 个逐字一致），`kekulized-generic-polycyclic-parent` 官方归因样本也仅 3/137 被命名；Step 3 与发版退出条件仍未达到；
